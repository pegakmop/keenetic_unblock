...
ndmc -c "show ip route" | grep Wireguard1 | awk ' { print "ndmc -c \"no ip route", $1, "\"" }'  | awk '{system($0)}'
...
while read -r addr || [ -n "$addr" ]; do
  ...
  for ipaddr in $(nslookup "$addr" | grep -v localhost | grep -v "::" | grep Address | awk '{print $3}' ); do
    echo "ip route $ipaddr/32 Wireguard1 !$addr" | awk ' { print "ndmc -c \"", $0, "\"" } ' | awk ' { system($0) } '
  done
